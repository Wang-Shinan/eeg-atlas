...
def build():
    ...
    def asset(url):
        ...
        mime={'.svg':'image/svg+xml','.png':'image/png','.jpg':'image/jpeg','.webp':'image/webp','.json':'application/json','.ico':'image/x-icon'}.get(p.suffix)
    ...
    for p in pages:
        ...
        # Asset URLs may appear in custom data attributes; replace the known data endpoints.
    ...
    (OUT/'README.txt').write_text('从 index.html 打开整套离线预览，建议使用当前 Chrome / Edge。所有内容来自实际构建；不代表已经发布到线上。第四、五章为合成教学演示。文献外链仍需网络。正文无需 JavaScript，交互需要 JavaScript。\n')
